...
import { buildSocialDraft } from "@/lib/social/service";
...
export async function POST(_request: Request, context: Params) {
  try {
    ...
    const result = await buildSocialDraft(id, channel as SocialChannel);
    if (!result.ok) {
    ...
    const message = error instanceof Error ? error.message : "Failed to build social draft.";
