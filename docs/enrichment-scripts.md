# Enrichment Scripts (Current)

This file contains the full current scripts used for enrichment operations.

## Active model setup

The enrichment pipeline reads model names from environment variables first, then falls back to the defaults in `lib/ai/openai-client.ts`.

Use this model setup for programmatic enrichment:

```bash
OPENAI_ENRICHMENT_MODEL=gpt-5.4
...
```bash
npm run enrich:pages -- --silo=zapier --limit=2 --offset=0 --dry-run=true
```

Write a reviewed batch:

```bash
npm run enrich:pages -- --silo=zapier --limit=15 --offset=0 --dry-run=false
```

## Static page enrichment

Static marketing pages use the same classifier -> writer brief -> JSON enrichment -> QA -> preview flow as the
programmatic enrichment script, but they do not run web research. The script reads and writes
`content/generated/static-pages.json`, then routes consume that file for visible copy, SEO metadata, and schema.

Smoke test all static pages without writes:

```bash
npm run enrich:static-pages -- --dry-run=true
```

Preview one page:

```bash
npm run enrich:static-pages -- --page=home --dry-run=true
```

Write reviewed static page enrichment:

```bash
npm run enrich:static-pages -- --dry-run=false
```

Review outputs in `reports/static-enrichment-preview/`, then run:
...
## `scripts/enrich-programmatic-pages.ts`
...
## `scripts/audit-page-similarity.ts`
...
## `scripts/export-enrichment-report.ts`
...
## Supporting modules used by the scripts

- `lib/ai/enrichment-prompts.ts`
- `lib/ai/enrichment-quality.ts`
- `lib/ai/enrichment-schemas.ts`
- `lib/ai/openai-client.ts`
- `lib/programmatic/local-enrichment.ts`
- `lib/programmatic/page-enrichment.ts`
- `lib/programmatic/render-body-html.ts`
- `lib/seo/display-names.ts`
- `lib/seo/tool-knowledge.ts`
- `lib/seo/tool-research.ts`
- `lib/seo/workflow-taxonomy.ts`
